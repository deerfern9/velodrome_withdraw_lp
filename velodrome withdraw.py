...
dai_weth_lp_contract = web3.eth.contract(address=web3.to_checksum_address(dai_weth_lp_address), abi=dai_weth_lp_abi)
...
def remove_liquidity_dai_weth(private, address):
    try:
        wrapped_eth_address = '0x4200000000000000000000000000000000000006'
        dai_address = '0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1'
        stable = False
        liquidity = dai_weth_lp_contract.functions.balanceOf(address).call()
        amount_a_min = int(1 / get_eth_price() * 4.9)
        amount_b_nin = int(4.9 * 1_000_000)
        to = address
        deadline = int(time.time() + 1800)
        approve(private, dai_weth_lp_contract)
        wait_normal_gwei()
        tx = velodrome_contract.functions.removeLiquidity(
            wrapped_eth_address, dai_address, stable, liquidity, amount_a_min, amount_b_nin, to, deadline
        ).build_transaction({
            'from': address,
            'nonce': web3.eth.get_transaction_count(address),
            'gasPrice': web3.eth.gas_price,
...
def main():
    ...
    for private in privates:
        ...
        elif dai_weth_lp_contract.functions.balanceOf(address).call() > web3.to_wei(0.00000001, 'ether'):
            if remove_liquidity_dai_weth(private, address):
                unwrap_eth(private, address, weth_contract)
                time.sleep(random.randint(*delay))
